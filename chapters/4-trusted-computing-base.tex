...
proving that the Verilog semantics are \emph{determinate}, the composition of
...
to programming languages, \textquote[{\textcite[pp. 699]{west10_cvd}}]{HDLs are
...
will result in the same final state.  The rule is triggered once for every clock
cycle.
...
\subsection{Determinate Verilog semantics}%
...
The final lemma we need is that the Verilog semantics is determinate. This
...
\begin{proof}[Proof sketch]
  The Verilog semantics is determinate because the order of operation of all the
  constructs is defined, so there is only one way to evaluate the module, and
  hence only one possible behaviour. This was proven for the small-step
