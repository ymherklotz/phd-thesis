...
%       \draw ($(0.5,\sw)+(0,0.\sz)+(0,1.4)$) -- ($(6.5,\sw)+(0,0.\sz)+(0,1.4)$);
  ...
  yellow respectively.]{\Gls{FPGA} layout, showing an example of a design that
    is placed and routed on the \gls{FPGA} highlighted in red.  The beige blocks
    correspond to \glspl{LUT}, followed by green blocks being \glspl{DSP} and
    purple blocks being \glspl{BRAM}.  The programmable interconnects are made
    up of connection blocks and switches shown in pink and blue
    respectively.}%
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \end{tikzpicture}
  \end{subfigure}
  \caption{Forward simulation diagrams, either performing one or more steps in
    the target program using the \enquote{plus} forward simulation, or
    performing zero or more steps in the target program with a well-founded
    order on the target program states.}%
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Comparison of the graph of trees structure and the block transfer
    language (\textsc{Btl}) structure, extending the comparisons shown in
    \cref{fig:block-comparison}.}%
