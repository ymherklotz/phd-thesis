...
\end{minted}
  ...
  \begin{subfigure}{0.6\linewidth}
    \centering
    \begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,>=Stealth,semithick]
      \node[state with output,initial] (S0) {$S_0$ \nodepart{lower} $0$};
      \node[state with output,below right=of S0] (S1) {$S_1$ \nodepart{lower} $0$};
      \node[state with output,above right=of S1] (S2) {$S_2$ \nodepart{lower} $1$};
      \path[->,every node/.style={font=\footnotesize}]
                (S0) edge [bend left] node [below left] {$0$} (S1)
                (S1) edge [bend left] node [below left] {$1$} (S0)
                (S0) edge [loop above] node [above] {$1$} (S0)
                (S1) edge node [below right] {$0$} (S2)
...
First, the \texttt{rst} input is checked, in which case the \texttt{state} variable is set back to \texttt{2'b0}.
Next, the state machine is implemented by checking the current value of \texttt{state} and defining the next \texttt{state}, including setting a value for the output \texttt{out}.
...
The concept of \gls{HLS} has also evolved over time, from describing the
...
their A64 ARMv8 ISA, because \textquote[{\textcite[sec~3.2,
  ...
  implementations}.  However, hyperblock scheduling has been a popular
...
On the other hand, \glspl{GPU} often have sophisticated support for predication so that the same sequence of instructions (also called warps) can be executed on multiple different threads on the \gls{GPU}, and so that diverging control-flow can still be handled by a single, large warp.
...
\textquote[{\textcite{jablin14_watsg}}]{Warp-Aware Trace Scheduling could benefit from trace-enlarging techniques, like Superblock scheduling's tail duplication and Hyperblock scheduling's predication}.
...
\begin{figure}
  ...
  \begin{tikzpicture}[xscale=1.7]
    ...
    \node[align=center] at (0,4) {\footnotesize Standard HLS tools \\
    ...
    \node[align=left] at (-1.7,-1.5) {\footnotesize K\^oika \\[-0.5em] \footnotesize\cite{bourgeat20_essen_blues}};
    \node[align=left] at (-2.4,0.2) { \footnotesize\citet{lööw21_lutsig}};

    \node at (2.2,-1.2) {\footnotesize\citet{ellis08_csicgfu}};
    \node at (-2.47,-0.7) { {\footnotesize\citet{perna12_mechan_wire_wise_verif_handel_c_synth}}};
...
Even though the correctness of the verifier is proven correct in the papers, the
...
\end{rtllisting}
\end{minipage}};
\node[bb,right=3.5cm of initial] (scheduled) {
...
\end{rtllisting}
  \end{minipage}
   ...
   \path (initial) -- node[pass] (fs) {scheduling} (scheduled);
   \draw[ed] (initial) -- (fs) -- (scheduled);
\end{tikzpicture}
\end{center}
