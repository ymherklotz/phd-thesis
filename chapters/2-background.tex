...
flow as net lists are essentially data flow graphs, and even \glspl{HDL} can be
...
\cref{sec:bg:static-scheduling} followed by describing dynamic scheduling in
\cref{sec:bg:dynamic-scheduling}.
...
tools~\cite{canis13_legup, xilinx20_vivad_high_synth, amd23_vitis_high_synth,
  ...
  roane23_autom_hw_sw_co_desig, mentor20_catap_high_level_synth} and means that
the time at which each operation will execute is known at compile time.  The
first step is to generate a \gls{CDFG}, which is either already the structure of
the code, or can be generated from a \gls{hyperblock} \gls{CFG} representation,
for example, by using static analysis on each \gls{hyperblock} to gather all
data dependencies and convert it into a \gls{DFG}.
...
uses an extension of \gls{SDC} scheduling~\cite[]{lattuada15_ctbsss} with
support for speculative execution and loop code motion.  In this dissertation,
\gls{SDC} will refer to the initial implementation of the scheduling without any
extensions.
...
described in \cref{sec:bg:interactive-theorem-provers}.  This section will give
a brief overview of the characteristics of these different verification tools.
...
characterised as decision procedures that will answer, if a formula is
satisfiable or not.  If the formula is satisfiable, the solver can provide a
model that will satisfy the formula.  Checking if the solver gave the right
answer is just a matter of plugging in the model to the formula and checking
that the formula is indeed satisfied.  \YH{TODO: Talk about unsatisfiability.}
