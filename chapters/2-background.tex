...
large semantic gap between small-step operational semantics of assembly and the
big-step operational Clight semantics.  The first three intermediate languages
(C\#minor, C\#minorgen, Cminor) are used to transform Clight, a deterministic
subset of C, into a more assembly like language called \gls{RTL}.  This language
consist of a \gls{CFG} of instructions, and is therefore well suited for various
compiler optimisations such as constant propagation, dead-code elimination or
function inlining.  After \gls{RTL}, each intermediate language is used to get
closer to the assembly language of the architecture, performing operations such
as register allocation.  \Cref{fig:bg:compcert-languages} gives a summary of
each transformation that takes place in CompCert, from the input C language to
the final native code that is run on the \gls{CPU}.
...
CompCert proofs follow two main patterns.  The first type of correctness proof
is a direct proof that the algorithm implemented in Coq is correct.  These types
of proofs do not carry any run time cost, because they reason purely about the
algorithm that was implemented and they can be erased when the C compiler is
...
programs written in Assembly.  This is done in CompCert using either big-step or
small-step operational semantics.  Big-step semantics are mainly used to define
the execution for languages in the front end of CompCert, whereas small-step
semantics are used in the back end of CompCert, and is therefore the most
relevant to this dissertation, so it is assumed that the small-step semantic
framework of CompCert is used.

\paragraph{Small-step semantics}

Small-step semantics in CompCert are defined as state transitions that possibly
emit observable events $t$.  A transition between states $s$ and $s'$, emitting
events $t$, is denoted as a single step transition
$s \oset{t}{\longrightarrow} s'$.  In addition to that, one needs to define an
initial state and a final state.  Finally, each program is also associated with
a global environment.  CompCert defines a small-step semantics framework that
can be reused by most languages, defining additional useful constructs such as
the reflexive, transitive closure of $\longrightarrow$ denoted as
$\longrightarrow^{*}$ or the transitive \enquote{plus} closure denoted as
$\longrightarrow^{+}$.  A semantics for a CompCert language is created by
defining the type for the state, the definition for a step in the semantics,
defining the initial and final states and the global environment of the program.

\paragraph{Program behaviour}

Once the semantics have been defined, the global behaviour of the program can be
defined, which will be used to express the correctness theorem.

\paragraph{Correctness theorem}
...
Outside of the realm of mechanised proof, most \gls{HLS} tools implement some form of
...
scheduling.  Other \gls{HLS} tools, such as
...
Several \gls{HLS} tools are associated with translation validators, either for an
individual scheduling
pass~\cite{chouksey20_verif_sched_condit_behav_high_level_synth,
  karfa06_formal_verif_method_sched_high_synth, kim04_autom_fsmd} or for the \gls{HLS}
