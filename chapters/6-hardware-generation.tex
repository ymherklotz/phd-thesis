...
that it can be implemented as an \gls{BRAM}, and will resort to using lots of registers
instead, ruining the circuit's area and performance.  This is because reads and
writes to a \gls{BRAM} need to follow a certain pattern to be suitable to be replaced
by \gls{BRAM} reads and writes.  For example, the synthesis tool will have to check
...
toggles the \texttt{u\_en} flag. This activates the \gls{BRAM} interface (lines 9--15
...
tool will correctly infer a \gls{BRAM} block.\footnote{Interestingly, the Verilog code
  shown for the \gls{BRAM} interface must not be modified, because the synthesis tool
  will only generate a \gls{BRAM} when the code matches a small set of specific
  patterns.}
...
Secondly, the logic in the enable signal of the \gls{BRAM} (\texttt{en != u\_en}) is
...
Fig.~\ref{fig:ram_load_store} gives an example of how the \gls{BRAM} interface behaves
...
value of \texttt{u\_en} on the next clock cycle, the \gls{BRAM} would be enabled again.
...
\htl{} can only represent a single state machine, so we must model the \gls{BRAM}
abstractly to reason about the correctness of replacing the direct read and
writes to the array by loads and stores to a \gls{BRAM}.  The specification for the \gls{BRAM}
is shown in Fig.~\ref{fig:htl_ram_spec}, which defines how the \gls{BRAM} $r$ will
...
machine.  However, as the \gls{BRAM} is triggered by the falling edge of the clock, it
...
Another simulation proof is performed to prove that the insertion of the \gls{BRAM} is
...
invariants needed for the simulation of the \gls{BRAM} insertion are quite different to
...
\newcommand\initialstate[1]{{#1}^{\mathrm I}_{\mathrm r}}
...
\begin{figure}
  ...
  \begin{tikzpicture}[>=Latex,shorten >=1pt,
    ...
    \node[bb] (initial block) {
      {\tiny 1 $\quad$}\mathstate{\texttt{assume:} $\at{\initialstate{\asgnblockingbefore}}{\texttt{state} \mapsto \varnothing; \texttt{r2} \mapsto
          \varnothing; \texttt{r8} \mapsto \varnothing}$}\\[0.5em]
      {\tiny 2 $\quad$}\veriloginline`state = 32'd2;`\\[0.5em]
      {\tiny 3 $\quad$}\mathstate{$\interstate{1}{\asgnblockingbefore} = \at{\initialstate{\asgnblockingbefore}}{\texttt{state} \mapsto
          \texttt{32'd2}}$}\\[0.5em]
      {\tiny 4 $\quad$}\mathstate{$\interstate{1}{\asgnnonblockingbefore} = \initialstate{\asgnnonblockingbefore}$}\\[0.5em]
      {\tiny 5 $\quad$}\veriloginline`r2 = r4 * r6;`\\[0.5em]
      ...
      {\tiny 8 $\quad$}\mathstate{$\interstate{2}{\asgnblockingbefore} = \at{\interstate{1}{\asgnblockingbefore}}{\texttt{r2} \mapsto
          \at{\initialstate{\asgnblockingbefore}}{\texttt{r4}} \mathbin{\texttt{*}}
          \at{\initialstate{\asgnblockingbefore}}{\texttt{r6}}}$}\\[0.5em]
      {\tiny 9 $\quad$}\mathstate{$\interstate{2}{\asgnnonblockingbefore} = \initialstate{\asgnnonblockingbefore}$}\\[0.5em]
      {\tiny 10 $\quad$}\veriloginline`r8 = r2 + r10;`\\[0.5em]
...
{\tiny 13 $\quad$}\mathstate{$\interstate{3}{\asgnblockingbefore} = \at{\interstate{2}{\asgnblockingbefore}}{\texttt{r8} \mapsto
          (\at{\initialstate{\asgnblockingbefore}}{\texttt{r4}} \mathbin{\texttt{*}}
          \at{\initialstate{\asgnblockingbefore}}{\texttt{r6}}) \mathbin{\texttt{+}}
          \at{\initialstate{\asgnblockingbefore}}{\texttt{r10}}}$}\\[0.5em]
{\tiny 14 $\quad$}\mathstate{$\interstate{3}{\asgnnonblockingbefore} = \initialstate{\asgnnonblockingbefore}$}\\[0.5em]
      {\tiny 15 $\quad$}\veriloginline`state = r3 ? 32'd3 : state;`\\[0.5em]
...
{\tiny 18 $\quad$}\mathstate{$\interstate{4}{\asgnblockingbefore} =
  \at{\interstate{3}{\asgnblockingbefore}}{\texttt{state} \mapsto
    \mternary{\at{\initialstate{\asgnblockingbefore}}{\texttt{r3}}}{\texttt{32'd3}}{\texttt{32'd2}}}$}\\[0.5em]
{\tiny 19 $\quad$}\mathstate{$\interstate{4}{\asgnnonblockingbefore} = \initialstate{\asgnnonblockingbefore}$}};
    \node[bb, right=1cm of
    initial block] (transf block)
    {\veriloginline`state <= 32'd2;`\\[0.5em]
     ...
     \veriloginline`r8 <= {r4 * r6} + r10;`\\[0.5em]
     ...
     \veriloginline`state <= r3 ? 32'd3 : 32'd2;`\\[0.5em]
...
\mathstate{$\interstate{4}{\asgnnonblockingafter} =
  ...
  \caption{Simple forward substitution transformation together with the runtime
    ...
    the nonblocking assignment association map ($\Delta_{\mathrm r}$).\YH{TODO:
      Need to find a better way to lay this out.}}%
...
{\Gamma'}^{\mathrm I}_{\mathrm r} \verilogmerge {\Delta'}^4_{\mathrm r}$. \YH{TODO:
  Need to fix the naming scheme as currently one cannot distinguish between the
  runtime maps before and after substitution.}
